...
export default function Todo({ category, title, pr, status, setTodos, id, todos, checked, setShowTodos }) {

    const [value, setValue] = useState(checked)

    return (
        <div className='row p-4 bg-white rounded-15p mt-4 align-items-center'>
            <p className='col-lg-4 col-md-4 col-sm-4 col-xs-12 col-6 hover-gold fw-600 pointer'>{title}</p>
            ...
            {status === "on" ? (
                ...
                : (<button
                    className='col-lg-2 col-md-2 col-sm-2 col-xs-6 col-6 my-1 btn btn-sm border-gold rounded-25p fs-12'>PERSONAL</button>)}

...
export default function Survey() {
    ...
    const [showSurvey, setShowSurveys] = useState([
        { title: "Developer Survey", category: "Development", status: "off", id: 458712, checked: 0 },
        { title: "Designer Survey", category: "Workplace", status: "on", id: 812712, checked: 1 },
        { title: "Backend Survey", category: "Hardware", status: "on", id: 172831, checked: 0 },
    ])
    ...
    const deleteHandler = (e) => {
        let arrey = survey.filter(todo => todo.checked == 0)
        setSurveys(arrey)
        setShowSurveys(arrey)
    }

    const filterHandler = (key, value) => {
        let arrey = survey.filter(todo => todo[key] == value)
        setShowSurveys(arrey)
    }

    const sortHandler = (key) => {
        console.log([...showSurvey].sort((a, b) => a.id - b.id));
        let arrey = [...showSurvey].sort((a, b) => a[key].localeCompare(b[key], undefined, { caseFirst: "upper" }))
        setShowSurveys(arrey)
    }
    ...
    return (
        <div>
            {showRightModal &&
                <RightModal title="New Survey" setShowRightModal={setShowRightModal}>
                    <div>
                        <div className="mb-4">
            ...
            <Breadcramp title="Survey" subTitle="Applicatins" sub="Survey" />
            <div onClick={() => setShowOptions(false)}
                className='main-container bg-main mt-3'>
                <div className='d-flex row'>
                    <div className='col-lg-4 col-md-4 my-1 d-flex'>
                ...
                <div className='row'>
                    <div className='col-lg-8 col-md-8 col-sm-12 py-3 '>
                        {showSurvey.map(item => (
                            <SurveyItem key={item.id} {...item} Survey={Survey} setSurveys={setSurveys} setShowSurveys={setShowSurveys} />
                        ))}
                    ...
                    <div className='col-lg-4 col-md-4 col-sm-12 ps-3 py-3 mt-4 '>
                        <div className='bg-white p-4 rounded-25p h-600'>
                            ...
                            <p className='mb-3 p-color4 fs-14'>StatusStatus</p>
                            <div onClick={() => setShowSurveys(survey)}
                                ...
                                <p className='ms-2 fs-14 '>All Surveys</p>
                                <p className='ms-auto fs-14'>{survey.length}</p>
                            </div>

                            <div onClick={() => filterHandler("checked", 0)}
                                ...
                                <p className='ms-2 fs-14'>Active Surveys</p>
                                <p className='ms-auto fs-14'>{survey.filter(t => t.checked == 0).length}</p>
                            </div>

                            <div onClick={() => filterHandler("checked", 1)}
                                className='d-flex align-items-center mb-3 hover-gold pointer'>
                                <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={.75} stroke="currentColor" className="svg-25 w-6 h-6">
                                    <path strokeLinecap="round" strokeLinejoin="round" d="M11.35 3.836c-.065.21-.1.433-.1.664 0 .414.336.75.75.75h4.5a.75.75 0 00.75-.75 2.25 2.25 0 00-.1-.664m-5.8 0A2.251 2.251 0 0113.5 2.25H15c1.012 0 1.867.668 2.15 1.586m-5.8 0c-.376.023-.75.05-1.124.08C9.095 4.01 8.25 4.973 8.25 6.108V8.25m8.9-4.414c.376.023.75.05 1.124.08 1.131.094 1.976 1.057 1.976 2.192V16.5A2.25 2.25 0 0118 18.75h-2.25m-7.5-10.5H4.875c-.621 0-1.125.504-1.125 1.125v11.25c0 .621.504 1.125 1.125 1.125h9.75c.621 0 1.125-.504 1.125-1.125V18.75m-7.5-10.5h6.375c.621 0 1.125.504 1.125 1.125v9.375m-8.25-3l1.5 1.5 3-3.75" />
                                </svg>

                                <p className='ms-2 fs-14'>Completed Surveys</p>
                                <p className='ms-auto fs-14'>{survey.filter(t => t.checked == 1).length}</p>
                            </div>
                            <p className='mb-3 p-color4 fs-14 mt-5'>Categories</p>
                            <div className="form-check fs-14">
                                <input onChange={() => filterHandler("category", "Development")}
                            ...
                            <div className="form-check">
                                <input onChange={() => filterHandler("category", "Workplace")}
                            ...
                            <div className="form-check">
                                <input onChange={() => filterHandler("category", "Hardware")}
